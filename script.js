// ================================
// BASE URL NGROK (API folder!)
// ================================
const apiBase = "https://heeled-kyndall-unusefully.ngrok-free.dev/tani-pintar/api";
...
function initSwitchHandlers() {
    ...
    fanSwitch.addEventListener("click", () => {
        const state = fanSwitch.classList.contains("active") ? 0 : 1;
        updateControl({ fan: state });
    });

    aerSwitch.addEventListener("click", () => {
        const state = aerSwitch.classList.contains("active") ? 0 : 1;
        updateControl({ aerator: state });
    });
}

// ================================
// KIRIM CONTROL KE SERVER
// ================================
